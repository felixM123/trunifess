...
@Component({
  selector: 'app-egreso-dinero',
  templateUrl: './egreso-dinero.component.html',
  styleUrls: ['./egreso-dinero.component.scss'],
  providers: [Literal]
})
export class EgresoDineroComponent implements AfterViewInit,OnInit {
  ...
  constructor(private calendar: NgbCalendar,
              ...
              private msj: MensajesService,private excelService:ExcelService,private lit: Literal,
              private agenciaService: AgenciaService,private crypto: CryptoService
  ) {
  ...
  listar() {
   ...
   listado() {
     if (this.data.tipo == 'TODAS') {
       ...
       this.data.tipo_transaccion = "SALIDA DE EFECTIVO";
       this.listar();
       setTimeout(() => {
         this.data.tipo = 'TODAS';
       }, 1000);
   ...
   this.excelService.exportAsExcelFile(this.expor, 'SALIDA DE EFECTIVO');
 }
 obtenerAgencias() {
