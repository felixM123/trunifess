...
@Component({
  selector: 'app-anular-recepcion',
  templateUrl: './anular-recepcion.component.html',
  styleUrls: ['./anular-recepcion.component.scss']
})
export class AnularRecepcionComponent implements AfterViewInit, OnInit {
  ...
  buscar() {
    ...
    this.recepcionService.reporteVerificacionAnulacion(this.anular1, this.type, this.token).subscribe(res => {
  ...
  eliminar() {
    if (this.guia.observacion == '') {
      this.msj.mensajeAdvertencia('Advertencia', 'Es obligatorio el campo observacion....');
    } else {
      Swal({
        title: 'Advertencia',
        text: 'Esta seguro de anular esta recepcion!',
        type: 'warning',
